<div class="content_box">
    ...
    <div class="box_body">
        ...
        <?php if (isset($catalog_lists) && count($catalog_lists) >= 1) : ?>
            <table class="table1 wd100pc" cellpadding="0" cellspacing="0">
                ...
                <?php foreach ($catalog_lists as $key => $val) : ?>

                    <tr style="<?= $key % 2 == 0 ? 'background: rgba(var(--Col_theme-main3), 0.25);' : ''; ?>">
                        <td class="p0c5 mb_hide">
                            <img src="<?= cdnURL("image/{$val['image_path'][0]['path']}"); ?>" alt="" style="width: 50px; height: 50px; object-fit: cover;">
                        </td>
                        <td class="p0c5">
                            <?= $val['catalog_name']; ?>
                        </td>
                        <td class="p0c5">
                            <div class="flex y_center h_center mb_flex_col" style="height: 100%;">
                                <a class="button1 bt_small p0" style="--bt_bg: none; --bt_color: rgb(var(--Col_theme-main)); --bt_border_color: none;" href="<?= adminURL("catalog/product/list/edit/{$val['catalog_id']}"); ?>">
                                    <i class="ri-edit-box-line mr0c5"></i> Edit
                                </a>

                                <div class="mt0c5 dk_hide tb_hide"></div>

                                <a class="del_cat button1 bt_small p0 ml1" data-id="<?= $val['catalog_id']; ?>" style="--bt_bg: none; --bt_color: rgb(var(--Col_ctx-erro)); --bt_border_color: none;">
    ...
    $('body')
        ...
        .on('click', '.del_cat', function() {
            ...
            if (action) {
                ...
                let url = $.makeURL.api().addPath(`catalog/product/${id}`).href,
